                        ...
                        <div class="col-md-5 align-self-center">
                            ...
                            <div class="scroll-defect-area-img" wire:loading.remove wire:target='loadReworkPage'>
                                <div class="all-defect-area-img-container">
                                    ...
                                    @endforeach
                                    @if ($allDefectImage)
                                        <img src="http://10.10.5.62:8080/erp/pages/prod_new/upload_files/{{ $allDefectImage->gambar }}" class="all-defect-area-img" id="all-defect-area-img-rework" alt="defect image">
                                    @else
                                        <img src="/assets/images/notfound.png" class="all-defect-area-img" id="all-defect-area-img-rework" alt="defect image">
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <div class="col-md-6">
            <div class="card h-100">
                <div class="card-header d-flex justify-content-between align-items-center bg-rework text-light">
                    ...
                    <div class="d-flex justify-content-end align-items-center gap-1">
                        <button type="button" class="btn btn-dark" wire:click="$emit('preSubmitUndo', 'rework')" disabled>
                            <i class="fa-regular fa-rotate-left"></i>
                        </button>
                        {{-- <button type="button" class="btn btn-dark">
                            <i class="fa-regular fa-gear"></i>
                        ...
                        @else
                            @foreach ($reworks as $rework)
                                <tr>
                                    ...
                                    <td>{{ $rework->kode_numbering }}</td>
                                    <td>{{ $rework->so_det_size }}</td>
                                    <td>{{ $rework->defectType->defect_type}}</td>
                                    <td>{{ $rework->defectArea->defect_area }}</td>
                                    <td class="text-rework fw-bold">{{ strtoupper($rework->status) }}</td>
                                    <td>
                                        <button type="button" class="btn btn-dark" wire:click="showDefectAreaImage('{{$rework->masterPlan->gambar}}', {{$rework->defect_area_x}}, {{$rework->defect_area_y}})'">
                                            <i class="fa-regular fa-image"></i>
                                        </button>
                                    </td>
                                    <td>
                                        ...
                                        <div wire:loading.remove>
                                            <button class="btn btn-sm btn-defect fw-bold w-100" wire:click="$emit('preCancelRework', '{{ $rework->id }}', '{{ $rework->id }}', '{{ $rework->so_det_size }}', '{{ $rework->defectType->defect_type }}', '{{ $rework->defectArea->defect_area }}', '{{$rework->masterPlan->gambar}}', {{$rework->defect_area_x}}, {{$rework->defect_area_y}})">CANCEL</button>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
